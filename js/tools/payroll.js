...
(() => {
  ...
  async function renderStats(items) {
    ...
    return `
      <div class="stat-strip">
        <div class="stat stat--electric"><span class="stat__label">Hours on clock NOW</span><span class="stat__value stat__value--electric">${liveHours.toFixed(1)}</span></div>
        ...
        <div class="stat stat--amber"><span class="stat__label">Next pay</span><span class="stat__value stat__value--amber" style="font-size:1.2rem">${nextPay ? Atlas.fmt.money(nextPay.gross) : '—'}</span></div>
        <div class="stat stat--green"><span class="stat__label">Paid (month)</span><span class="stat__value stat__value--green">${Atlas.fmt.money(mtdGross)}</span></div>
      </div>
    `;
  }
  ...
  Atlas.registerRenderer('payroll', async function (root) {
    ...
    function paintList() {
      ...
      listEl.querySelectorAll('[data-pay]').forEach(b => b.addEventListener('click', async () => { await DataStore.update(COLLECTION, b.dataset.pay, { status: 'paid' }); UI.toast('Period closed', 'success'); }));
    }
    async function reload() { items = await DataStore.list(COLLECTION); root.querySelector('#stats-slot').innerHTML = await renderStats(items); paintList(); }
    Atlas.onData(COLLECTION, reload);
    // Also live-update when the time clock ticks
